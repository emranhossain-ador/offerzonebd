...
class DrivePackage extends Component
{
    public $username;
    public $operator = 'gp';
    public $type = 'all';
    ...
    public function confirmPurchase()
    {
        ...
        if($insertData){
            ...
            Notifications::create([
                'user_id' => _auth()->id,
                'title' => 'Drive Package Order',
                'service_id' => $insertData->id,
                'role' => 'admin',
                'type' => 'order',
            ]);

            session()->flash('success', 'Drive Package Order Successfully');
            return redirect()->route('user.home', ['username' => $this->username]);
        }else{
            session()->flash('error', 'Failed to purchase Drive Package');
            return redirect()->route('user.home', ['username' => $this->username]);
        }
    ...
    public function render()
    {
        ...
        return view('livewire.frontend.user.drive-package', $data);
    }
}
