<div class="main-content">
    <div class="sticky z-20 top-0 w-full">
        <div class="w-full border-b border-border bg-card py-3.5 md:py-5 px-2.5 md:px-4 shadow-sm">

            <div class="flex items-center gap-1.5 justify-evenly pb-5">

                <button wire:click="filterByOperator('gp')" type="button"
                    class="w-14 h-14 md:w-16 md:h-16 shrink-0 rounded-md border p-2 cursor-pointer flex justify-center items-center {{ $operator == 'gp' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary' }}">
                    <img src="{{ asset('assets/images/gp-filter.png') }}" alt="logo"
                        class="w-full h-full object-contain">
                </button>

                <button wire:click="filterByOperator('robi')" type="button"
                    class="w-14 h-14 md:w-16 md:h-16 shrink-0 rounded-md border p-2 cursor-pointer flex justify-center items-center shadow-md {{ $operator == 'robi' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary' }}">
                    <img src="{{ asset('assets/images/robi.png') }}" alt="logo"
                        class="w-full h-full object-contain">
                </button>

                <button wire:click="filterByOperator('airtel')" type="button"
                    class="w-14 h-14 md:w-16 md:h-16 shrink-0 rounded-md border p-2 cursor-pointer flex justify-center items-center shadow-md {{ $operator == 'airtel' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary' }}">
                    <img src="{{ asset('assets/images/airtel-filter.png') }}" alt="logo"
                        class="w-full h-full object-cover">
                </button>

                <button wire:click="filterByOperator('bl')" type="button"
                    class="w-14 h-14 md:w-16 md:h-16 shrink-0 rounded-md border p-2 cursor-pointer flex justify-center items-center shadow-md {{ $operator == 'bl' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary' }}">
                    <img src="{{ asset('assets/images/bl-filter.png') }}" alt="logo"
                        class="w-full h-full object-contain">
                </button>

                <button wire:click="filterByOperator('teletalk')" type="button"
                    class="w-14 h-14 md:w-16 md:h-16 shrink-0 rounded-md border p-2 cursor-pointer flex justify-center items-center shadow-md {{ $operator == 'teletalk' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary' }}">
            ...
            <div
                class="flex items-center justify-between md:justify-center gap-1 md:gap-2 p-1 md:p-1.5 mt-5 bg-card rounded-md border border-border md:w-fit mx-auto shadow-sm divide-x divide-border dark:divide-gray-700 overflow-x-auto w-full whitespace-nowrap">

                <button wire:click="filterByType('all')"
                    class="flex justify-center w-full md:w-fit md:items-center gap-1 flex-row px-2.5 py-1 cursor-pointer text-sm {{ $type == 'all' ? 'bg-primary text-primary-foreground rounded-md' : 'bg-card text-foreground transition-all hover:text-primary ' }}">
                    <i class="ri-stack-line"></i>
                    <span>All</span>
                </button>

                <button wire:click="filterByType('internet')"
                    class="flex justify-center w-full md:w-fit md:items-center gap-1 flex-row px-2.5 py-1  cursor-pointer text-sm {{ $type == 'internet' ? 'bg-primary text-primary-foreground rounded-md' : 'bg-card text-foreground transition-all hover:text-primary ' }}">
                    <i class="ri-wifi-line"></i>
                    <span>Internet</span>
                </button>

                <button wire:click="filterByType('minute')"
                    class="flex justify-center w-full md:w-fit md:items-center gap-1 flex-row px-2.5 py-1 cursor-pointer text-sm {{ $type == 'minute' ? 'bg-primary text-primary-foreground rounded-md' : 'bg-card text-foreground transition-all hover:text-primary ' }}">
                    <i class="ri-phone-line"></i>
                    <span>Minutes</span>
                </button>

                <button wire:click="filterByType('bundle')"
                    class="flex justify-center w-full md:w-fit md:items-center gap-1 flex-row px-2.5 py-1 cursor-pointer text-sm {{ $type == 'bundle' ? 'bg-primary text-primary-foreground rounded-md' : 'bg-card text-foreground transition-all hover:text-primary ' }}">
    ...
    <main class="px-1.5 md:px-3.5 py-4 space-y-3">
        @if (_adminSettingById(1)->is_drive_active == true)
            ...
            @foreach ($packages as $key => $package)
                <button type="button" wire:click="selectedPack({{ $package->id }})"
                    class="flex items-center w-full gap-3.5 cursor-pointer rounded-2xl border border-border bg-card/70 p-3 backdrop-blur-xl transition hover:border-primary/40 shadow-sm hover:shadow-none animate-fade-up"
                    ...
                    <div class="flex-1 text-left flex flex-col">
                        <p class="font-display text-sm font-semibold line-clamp-2">{{ $package->title }}</p>
    ...
    <div x-data x-cloak x-show="$wire.showSelectedOffer" x-transition:enter="ease-out duration-300"
        ...
        <div x-show="$wire.showSelectedOffer" x-transition:enter="ease-out duration-300"
            ...
            <form wire:submit="selectPackageSave">
                <!-- Body -->
                <div class="block px-4 py-4 space-y-7 w-full!">
                    ...
                    <div
                        ...
                        <div class="flex-1 text-left flex flex-col">
                            <p class="font-display text-sm font-semibold line-clamp-2">{{ $selectedpackage?->title }}
                            </p>

                            <div class="mt-1 flex w-full justify-between items-center gap-1.5 text-xs">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="rounded-full font-medium bg-gray-200/70 dark:bg-gray-300/10 px-2 py-1 text-foreground/60">{{ $selectedpackage?->validity }}
                                        দিন</span>
                                    <span
                                        class="rounded-full bg-primary-glow/15 px-2 py-0.5 font-semibold text-primary-glow truncate">{{ ucfirst($selectedpackage?->type) }}</span>
        ...
        <div x-show="$wire.orderConfirmModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="block px-4 py-4 space-y-7 w-full!">
                ...
                <div
                    ...
                    <div class="flex-1 text-left flex flex-col">
                        <p class="font-display text-sm font-semibold line-clamp-2">{{ $selectedpackage?->title }}</p>
                ...
                <div class="mb-3.5 rounded-xl bg-background/80 border border-border divide-y divide-border">
                    ...
                    <div class="flex items-center justify-between gap-3 p-3">
                        <p class="shrink-0 text-xs text-foreground/70">Package</p>
                        <div class="min-w-0 text-right">
            ...
            <!-- Footer -->
            <div class="flex items-center gap-3 border-t border-border bg-card/50 px-5 py-4">
