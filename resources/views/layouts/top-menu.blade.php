<header class="relative lg:fixed left-0 w-full top-0 z-50 glass backdrop-blur-lg">
    <div x-data="{ open: false }"
        class="mx-auto flex max-w-7xl items-center justify-between gap-2 px-2 py-1 sm:px-5 sm:py-2">
            ...
            @if (auth()->check())
                @if (auth()->user()->role == 'admin')
                    <a href="{{ route('admin.dashboard') }}"
                        class="group flex items-center justify-center gap-2 overflow-hidden rounded-xl gradient-bg px-4 py-2 text-sm font-bold text-primary-foreground transition-all relative"
                        wire:navigate>
                        <span
                        ...
                        <i class="ri-slow-down-line"></i> Dashboard
                    </a>
                @else
        ...
        <!-- Only For Mobile -->
        <nav x-show="open" x-cloak x-collapse.duration.500ms
