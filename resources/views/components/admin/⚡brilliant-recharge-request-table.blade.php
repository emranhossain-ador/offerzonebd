...
new class extends Component {
    ...
    {
        return BrilliantRecharge::with('user')->latest()->paginate(30);
    }
    ...
    {
        $this->brilliantRecharge();
    }

    public function statusChange($id, $status)
    {
        $recharge = BrilliantRecharge::find($id);
        if ($recharge) {
            ...
            Notifications::create([
                'title' => $status == 'success' ? 'আপনার ব্রিলিয়ান্ট রিচার্জ রিকোয়েস্টটি সফল করা হয়েছে ✅' : 'আপনার ব্রিলিয়ান্ট রিচার্জ রিকোয়েস্টটি রিজেক্ট করা হয়েছে ❌',
                'user_id' => $recharge->user_id,
                'service_id' => $recharge->id,
                'type' => 'brilliant_recharge',
                'description' => $status == 'success' ? 'আপনার ৳' . $recharge->amount . ' ব্রিলিয়ান্ট রিচার্জ সফল হয়েছে। রিচার্জটি আপনার দেওয়া নম্বর ' . $recharge->brilliant_number . '-এ সম্পন্ন হয়েছে। অনুগ্রহ করে নম্বরটি চেক করুন।' : 'আপনার ৳' . $recharge->amount . ' ব্রিলিয়ান্ট রিচার্জ রিকোয়েস্টটি রিজেক্ট করা হয়েছে। অনুগ্রহ করে আপনার দেওয়া নম্বর ' . $recharge->brilliant_number . ' এবং রিচার্জের তথ্য যাচাই করে পুনরায় চেষ্টা করুন। ❌',
                'is_seen' => false,
            ]);
...
<div>
    @if ($this->brilliantRecharge()->isEmpty())
        <div class="text-center py-10">
            <i class="ri-search-line text-4xl text-muted-foreground"></i>
            <p class="mt-2 text-muted-foreground">No Brilliant Recharge Request found</p>
        </div>
    @else
        <div
            ...
            <table class="w-full table-auto">
                ...
                <thead>
                    <tr class="bg-muted divide-x divide-border text-foreground text-sm font-semibold text-left">
                        ...
                        <th class="px-3 py-4 whitespace-nowrap">Brilliant Number</th>
                ...
                <tbody>
                    ...
                    @foreach ($this->brilliantRecharge() as $key => $recharge)
                        <!-- Example Row -->
                        <tr wire:key="{{ $recharge->id }}"
                            ...
                            <td class="px-3 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <span
                                        class="text-sm font-semibold bg-primary text-white px-2 py-1 rounded shadow-sm">{{ $recharge->brilliant_number }}</span>
                                    <button type="button"
                                        ...
                                        data-copy="{{ $recharge->brilliant_number }}">
                            ...
                            <td class="px-3 py-4 text-center whitespace-nowrap">
                                <div class="flex gap-2 justify-center">
                                    ...
                                    <button onclick="confirmDelete({{ $recharge->id }}, 'brilliant_recharges')"
